...
$project = [Xml](Get-Content "..\CentennialPackaging\CentennialPackaging.wapproj" -Raw)
$appxBundle = ($project.Project.PropertyGroup.AppxBundle | Out-String).Trim().Split(@(" ","`r","`n","`t"))[0]
...
if ((-not [string]::IsNullOrEmpty($appxBundle)) -and (-not [string]::IsNullOrEmpty($appxBundlePlatforms))) {
	...
	InflateDataFile "..\CentennialPackaging\Data.dat" $inflationSize
}

# Create the list of language resources
CreateLanguageResourcesForBridge ".\Strings" $settings.Languages